...
int main( const int argc, const char** argv )
{
  ...
  if ( argc != 3 )
  {
    std::cout << "Usage: " << argv[0] << " <fn>\n";
    std::cout << "    where fn is one of WFG1, ..., WFG9, I1, ..., I5, b_poly, ..., r_nonsep\n";
    return 1;
  }
  ...
  {
    const int count = atoi(argv[2]);  // the number of random fitness values to print
    const int M = 2;          // the number of objectives
    const int k_factor = 4;   // k (# position parameters) = k_factor*( M-1 ) 
    const int l_factor = 10;   // l (# distance parameters) = l_factor*2
